...
<body>
    <div id="formulario" style="margin: 50px;">
        <form action="#" method="POST">
            <label>
               Filtrar alumnos suspensos
            </label>
            <select name="beca">  <!--seleccionamos por lo que queremos filtrar -->
                <?php
                     $consulta = $dbh->prepare("SELECT DISTINCT alumnos.nom_alum,matriculas.calificacion FROM alumnos,matriculas");
                     ...
                     foreach($becas as $dato){
                        echo "<option value=" . $dato["calificacion"] . ">" . $dato["calificacion"] . "</option>";
                     }
                ?>
            </select>
